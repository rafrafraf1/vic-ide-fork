...
export function writeMemory(computer: ComputerState, address: Address, value: Value): void {
...
export function executeInstruction(computer: ComputerState, instruction: Instruction, nextInput: Value): ExecuteResult {
    switch (instruction.kind) {
        case "ADD":
            computer.dataRegister = add(computer.dataRegister, readMemory(computer, instruction.address));
            computer.programCounter++;
            return nullExecuteResult;
        case "SUB":
            computer.dataRegister = sub(computer.dataRegister, readMemory(computer, instruction.address));
            computer.programCounter++;
            return nullExecuteResult;
        case "LOAD":
